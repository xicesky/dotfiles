...
install_prereqs() {
    ...
    invoke sudo apt-get install -y git etckeeper python3-pip || return 1
}

install_tools_debian() {
    invoke sudo apt-get install -y \
        vim p7zip-full lzma pigz gzrt gzip bzip2 \
        mc curl wget nmap hwinfo strace htop iotop iptraf-ng tcpdump \
...
cmd_install() {
    ...
    invoke "$DOTFILES_DIR/installer/install.sh" install-only
